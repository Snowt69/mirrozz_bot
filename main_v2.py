...
SUBGRAM_MAX_OP = 6
ADMINS = [7057452528, 7236484299, 6634823286, 8153569100]
...
async def check_subgram_subscription_callback(callback: types.CallbackQuery):
    ...
    await update_user_activity(user_id)
    link_id = callback.data.split("_")[-1] if callback.data != "check_subgram_subscription_start" and callback.data != "check_subgram_subscription_help" else None
    
    # Проверяем подписку на SubGram
    status, result = await check_subgram_subscription(
        user_id=user_id,
        chat_id=callback.message.chat.id,
        first_name=callback.from_user.first_name,
    ...
    if status == 'ok':
        # Очищаем кэш
        subgram_cache.pop(cache_key, None)
        subscription_cache.pop(f"{user_id}_False", None)
        subscription_cache.pop(f"{user_id}_True", None)
        ...
        try:
            ...
            logger.error(f"Failed to delete message for user {user_id}: {str(e)}")
        
        # Выполняем дальнейшую логику
        if link_id and link_id != "start":
            await handle_link_visit(callback.message, link_id)
        elif callback.data == "check_subgram_subscription_help":
            text = (
                f"📚 <b>Команды Mirrozz Scripts</b>\n\n"
                f"👋 /start — Запустить бота и получить приветствие\n"
                f"📊 /user_stats — Посмотреть статистику твоего профиля\n"
        ...
        for link in result:
            ...
            keyboard.add(button)
        callback_data = f"check_subgram_subscription_{link_id}" if link_id else "check_subgram_subscription_start"
        if callback.data == "check_subgram_subscription_help":
            callback_data = "check_subgram_subscription_help"
        keyboard.add(InlineKeyboardButton(text="✅ Я подписался", callback_data=callback_data))
        await edit_message_if_changed(
            callback=callback,
            text=(
                f"📢 <b>Проверка на подписки</b>\n\n"
                f"Вы не подписаны на все необходимые каналы. Подпишитесь на каналы в первом сообщении, затем нажмите 'Я выполнил' и нажмите на кнопку 'Я подписался' в этом сообщении:"
            ),
            reply_markup=keyboard
        )
        await callback.answer("Пожалуйста, подпишитесь на все каналы и нажмите 'Я подписался'.")
        return
        ...
        with sqlite3.connect(file_path) as conn:
            ...
            if cursor.fetchone()[0] != 'ok':
                ...
                await send_message_safe(
                    bot=bot,
                    chat_id=message.chat.id,
                    text="⚠️ Загруженный файл базы данных поврежден."
                )
                await state.finish()
                return
