...
async def handle_callback_query(callback: types.CallbackQuery):
    ...
    if data == "back_to_main":
        ...
        if not channels:
            ...
            text = "📢 <b>Список каналов</b>\n\n"
            for channel_id, title, check_scope in channels:
                scope = await scope_text(check_scope)
                text += f"📢 {title} ({scope})\n"
            await edit_message_if_changed(
                callback=callback,
                text=text,
        ...
        command = data.split("_")[-1] if link_id is None else None
        check_all_functions = not link_id
        unsubscribed_local = await check_subscriptions(user_id, check_all_functions)
        ...
        if unsubscribed_local:
            channel_list = "\n".join([f"👉 {name}" for name in unsubscribed_local])
            keyboard = InlineKeyboardMarkup()
            callback_data = f"check_local_subscription_{link_id}" if link_id else f"check_local_subscription_{command}"
            keyboard.add(InlineKeyboardButton(text="✅ Я подписался", callback_data=callback_data))
            await edit_message_if_changed(
                callback=callback,
                text=(
        ...
        if status == 'need_subscribe':
            ...
            for link in result:
                ...
                button = InlineKeyboardButton(text=f"Подписаться на {channel_name}", url=url)
                keyboard.add(button)
            callback_data = f"check_subgram_subscription_{link_id}" if link_id else f"check_subgram_subscription_{command}"
            keyboard.add(InlineKeyboardButton(text="✅ Я подписался", callback_data=callback_data))
            await edit_message_if_changed(
                callback=callback,
                text=(
        ...
        # Повторная проверка подписок после указания пола
        status, result = await check_subgram_subscription(
            user_id=user_id,
            chat_id=callback.message.chat.id,
            first_name=callback.from_user.first_name,
        ...
        banned_user_id = int(data.split("_")[3])
        ban = await execute_with_retry_async("SELECT admin_id, reason, banned_at FROM bans WHERE user_id = ?", (banned_user_id,))
        if not ban:
            await edit_message_if_changed(
                callback=callback,
        ...
        ban = await execute_with_retry_async("SELECT reason, banned_at FROM bans WHERE user_id = ?", (target_user_id,))
        ban_info = f"\n🚫 Статус бана: <b>{'Забанен' if ban else 'Не забанен'}</b>"
        if ban:
            reason, banned_at = ban[0]
            ban_date = datetime.fromtimestamp(banned_at).strftime("%d.%m.%Y %H:%M")
            ban_info += f"\n💬 Причина: <b>{reason}</b>\n📅 Дата бана: <b>{ban_date}</b>"
        text = (
            ...
            f"🔗 Переходов по ссылкам: <b>{visits}</b>"
            f"{ban_info}"
        )
        await edit_message_if_changed(
            callback=callback,
