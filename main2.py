...
async def handle_callback_query(callback: types.CallbackQuery):
    ...
    if data == "back_to_main":
        ...
        if not channels:
            ...
            for channel_id, title, check_scope in channels:
                scope_text = await scope_text(check_scope)
                text += f"📢 {title} ({scope_text})\n"
            await edit_message_if_changed(
                callback=callback,
                text=text,
                reply_markup=await create_back_keyboard()
        ...
        await edit_message_if_changed(
            callback=callback,
            text=f"⚠️ <b>Вы уверены, что хотите удалить канал {channel_id}?</b>",
        ...
        await edit_message_if_changed(
            callback=callback,
            text=f"✅ Канал {channel_id} удален.",
            reply_markup=await create_back_keyboard()
        )
        ...
        await edit_message_if_changed(
            callback=callback,
            text=f"📩 <b>Отправка сообщения пользователю</b> {user_name}\n\nВведите текст сообщения:",
            reply_markup=await create_back_keyboard()
        )
        ...
        await edit_message_if_changed(
            callback=callback,
            text=f"🚫 <b>Блокировка пользователя</b> {user_name}\n\nУкажите причину бана:",
            reply_markup=await create_back_keyboard()
        )
        ...
        user_name = user[0][0] if user else f"Пользователь {target_user_id}"
        username = user[0][1] if user and user[0][1] else "Нет"
        admin = await execute_with_retry_async("SELECT first_name FROM users WHERE user_id = ?", (admin_id,))
        admin_name = admin[0][0] if admin else f"Админ {admin_id}"
        date = datetime.fromtimestamp(banned_at).strftime("%d.%m.%Y %H:%M")
        text = (
        ...
        check_type = data.split("_")[-1]
        check_all_functions = check_type in ["start", "help", "report", "stats"]
        unsubscribed_local = await check_subscriptions(user_id, check_all_functions)
        if unsubscribed_local:
            channel_list = "\n".join([f"👉 {name}" for name in unsubscribed_local])
            keyboard = InlineKeyboardMarkup()
            keyboard.add(InlineKeyboardButton(text="✅ Я подписался", callback_data=f"check_local_subscription_{check_type}"))
            await edit_message_if_changed(
                callback=callback,
                text=(
                    f"📢 <b>Подпишитесь на наши каналы</b>\n\n"
                    f"Для доступа к {'скрипту' if check_type not in ['start', 'help', 'report', 'stats'] else 'функциям бота'} подпишитесь на каналы:\n\n{channel_list}\n\n"
                    f"После подписки нажмите кнопку ниже:"
                ),
                reply_markup=keyboard
            )
        ...
        if status == 'need_subscribe':
            ...
            for link in result:
                ...
                button = InlineKeyboardButton(text=f"Подписаться на {channel_name}", url=url)
                keyboard.add(button)
            keyboard.add(InlineKeyboardButton(text="✅ Я подписался", callback_data=f"check_subgram_subscription_{check_type}"))
            await edit_message_if_changed(
                callback=callback,
                text=(
                    f"📢 <b>Подпишитесь на дополнительные каналы</b>\n\n"
        ...
        if check_type == "start":
            ...
            await handle_link_visit(callback.message, check_type)
        ...
        gender = "male" if data.endswith("male") else "female"
