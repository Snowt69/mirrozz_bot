...
async def check_subgram_subscription(user_id, chat_id, first_name=None, language_code="ru", is_premium=False, gender=None):
    ...
    try:
            ...
            async with client.post(SUBGRAM_API_URL, headers=headers, json=data, retry_options=retry_options) as response:
                ...
                if response.status == 200:
                    if result.get('status') == 'ok':
                        ...
                        subgram_cache[cache_key] = ('error', "Неизвестный статус от сервиса SubGram")
                        return 'error', "Неизвестный статус от сервиса SubGram"
                    ...
                    subgram_cache[cache_key] = ('error', f"Ошибка сервера SubGram (код {response.status})")
                    return 'error', f"Ошибка сервера SubGram (код {response.status})"
        ...
        subgram_cache[cache_key] = ('error', f"Ошибка SubGram: {str(e)}")
        return 'error', f"Ошибка SubGram: {str(e)}"
...
async def create_report_actions_keyboard(report_id):
    keyboard = InlineKeyboardMarkup(row_width=1)
    keyboard.add(
...
async def create_admins_keyboard():
    ...
    keyboard.add(
        ...
        InlineKeyboardButton(text="🔙 Назад", callback_data="section_admins")
    )
    return keyboard
...
async def create_channel_scope_keyboard():
    ...
    keyboard.add(
        ...
        InlineKeyboardButton(text="🔙 Назад", callback_data="section_ads")
    )
    return keyboard
...
async def create_channels_keyboard(action="remove_channel"):
    ...
    keyboard.add(InlineKeyboardButton(text="🔙 Назад", callback_data="section_ads"))
    return keyboard
...
async def handle_help(message: types.Message):
    ...
    if status == 'need_subscribe':
        ...
        await send_message_safe(
            bot=bot,
            chat_id=message.chat.id,
            text="⚠️ Произошла ошибка при проверки подписки. Попробуйте позже."
        )
        return
    ...
    if state_value == "awaiting_link_content" and await is_admin(user_id):
        ...
        try:
            ...
            await send_message_safe(
                bot=bot,
                chat_id=message.chat.id,
                text=f"✅ Пользователь с ID {new_admin_id} теперь администратор.",
...
async def handle_callback_query(callback: types.CallbackQuery):
    ...
    if data == "back_to_main":
        ...
        if state.get("state") != "awaiting_channel_scope":
            await edit_message_if_changed(
                callback=callback,
                text="⚠️ Неверное состояние. Пожалуйста, начните добавление канала заново.",
                reply_markup=await create_back_keyboard()
            )
            return
        channel_id = state.get("channel_id")
        check_scope = "links_only" if data == "scope_links_only" else "all_functions"
        ...
        scope_text = "только для ссылок" if check_scope == "links_only" else "все функции бота"
        await edit_message_if_changed(
            callback=callback,
            text=f"✅ Канал {title} добавлен для проверки подписки ({scope_text}).",
            reply_markup=await create_back_keyboard()
        )
        await dp.storage.set_data(user=user_id, data={})
    
    elif data == "remove_channel":
        if not (await execute_with_retry_async("SELECT COUNT(*) FROM channels"))[0][0]:
            await edit_message_if_changed(
                callback=callback,
        ...
        await edit_message_if_changed(
            ...
            reply_markup=await create_channels_keyboard("remove_channel")
        )
        ...
        if not channels:
            await edit_message_if_changed(
                callback=callback,
                text="📋 <b>Нет каналов для подписки</b>",
                reply_markup=await create_back_keyboard()
            )
            return
        channel_list = "\n".join([f"• {title} ({hcode(channel_id)}, {await scope_text(check_scope if check_scope else 'links_only')})" for channel_id, title, check_scope in channels])
        await edit_message_if_changed(
            callback=callback,
            text=f"📋 <b>Список каналов для подписки</b>\n\n{channel_list}",
            reply_markup=await create_channels_keyboard("channel_info")
        )
        ...
        await edit_message_if_changed(
            callback=callback,
            text=f"⚠️ <b>Вы уверены, что хотите удалить канал {title}?</b>",
        ...
        await edit_message_if_changed(
            callback=callback,
            text=f"✅ Канал {title} удален из списка подписки.",
            reply_markup=await create_back_keyboard()
        )
    
    elif data.startswith("channel_info_"):
        channel_id = data.split("_")[2]
        channel = await execute_with_retry_async("SELECT title, check_scope FROM channels WHERE channel_id = ?", (channel_id,))
        if not channel:
            await edit_message_if_changed(
                callback=callback,
                text="⚠️ Канал не найден.",
            ...
            return
        title, check_scope = channel[0]
        scope_text_val = await scope_text(check_scope if check_scope else 'links_only')
        await edit_message_if_changed(
            callback=callback,
            text=f"📢 <b>Информация о канале</b>\n\n"
                 f"Название: <b>{title}</b>\n"
                 f"ID: {hcode(channel_id)}\n"
                 f"Проверка подписки: <b>{scope_text_val}</b>",
            reply_markup=await create_back_keyboard()
        )
    
    elif data.startswith("check_local_subscription_"):
        link_id = data.split("_")[-1] if data != "check_local_subscription_start" else None
        check_all_functions = not link_id or link_id in ["help", "stats", "report"]
        unsubscribed_local = await check_subscriptions(user_id, check_all_functions)
        
        if unsubscribed_local:
            channel_list = "\n".join([f"👉 {name}" for name in unsubscribed_local])
            keyboard = InlineKeyboardMarkup()
            callback_data = f"check_local_subscription_{link_id}" if link_id and link_id not in ["help", "stats", "report"] else f"check_local_subscription_{data.split('_')[-1]}"
            keyboard.add(InlineKeyboardButton(text="✅ Я подписался", callback_data=callback_data))
            await edit_message_if_changed(
                callback=callback,
                text=(
                    f"📢 <b>Подпишитесь на наши каналы</b>\n\n"
                    f"Для доступа к {'скрипту' if link_id and link_id not in ['help', 'stats', 'report'] else 'функциям бота'} подпишитесь на каналы:\n\n{channel_list}\n\n"
                    f"После подписки нажмите кнопку ниже:"
                ),
                reply_markup=keyboard
            )
        ...
        if status == 'need_subscribe':
            ...
            for link in result:
                ...
                button = InlineKeyboardButton(text=f"Подписаться на {channel_name}", url=url)
                keyboard.add(button)
            callback_data = f"check_subgram_subscription_{link_id}" if link_id and link_id not in ["help", "stats", "report"] else f"check_subgram_subscription_{data.split('_')[-1]}"
            keyboard.add(InlineKeyboardButton(text="✅ Я подписался", callback_data=callback_data))
            await edit_message_if_changed(
                callback=callback,
                text=(
        ...
        if link_id and link_id not in ["help", "stats", "report"]:
            await handle_link_visit(callback.message, link_id)
        elif data == "check_local_subscription_start":
            welcome_text = (
        ...
        elif data == "check_local_subscription_help":
            await handle_help(callback.message)
        elif data == "check_local_subscription_stats":
            await handle_user_stats(callback.message)
        elif data == "check_local_subscription_report":
            await send_message_safe(
                bot=bot,
                chat_id=callback.message.chat.id,
                text="📩 <b>Отправка жалобы</b>\n\nПожалуйста, укажите сообщение жалобы: /report [сообщение]"
        ...
        if link_id and link_id not in ["help", "stats", "report"]:
            await handle_link_visit(callback.message, link_id)
        elif data == "check_subgram_subscription_start":
            welcome_text = (
                f"👋 <b>Привет, {callback.from_user.first_name}!</b>\n\n"
                f"Я <b>Mirrozz Scripts</b> — бот, который выдает актуальные скрипты и инжекторы для Roblox по ссылке! 🚀\n\n"
                f"<b>Почему я лучший?</b>\n"
        ...
        elif data == "check_subgram_subscription_help":
            await handle_help(callback.message)
        elif data == "check_subgram_subscription_stats":
            await handle_user_stats(callback.message)
        elif data == "check_subgram_subscription_report":
            await send_message_safe(
                bot=bot,
                chat_id=callback.message.chat.id,
                text="📩 <b>Отправка жалобы</b>\n\nПожалуйста, укажите сообщение жалобы: /report [сообщение]"
        ...
        # Повторная проверка SubGram после указания пола
        status, result = await check_subgram_subscription(
            user_id=user_id,
            chat_id=callback.message.chat.id,
            first_name=callback.from_user.first_name,
        ...
        if status == 'need_subscribe':
            ...
            for link in result:
                ...
                button = InlineKeyboardButton(text=f"Подписаться на {channel_name}", url=url)
                keyboard.add(button)
            callback_data = "check_subgram_subscription_start"  # По умолчанию возвращаемся к началу
            keyboard.add(InlineKeyboardButton(text="✅ Я подписался", callback_data=callback_data))
            await edit_message_if_changed(
                callback=callback,
                text=(
        ...
        user_name = (await execute_with_retry_async("SELECT first_name FROM users WHERE user_id = ?", (user_id,)))[0][0] if await execute_with_retry_async("SELECT first_name FROM users WHERE user_id = ?", (user_id,)) else f"Пользователь {user_id}"
        date = datetime.fromtimestamp(created_at).strftime("%d.%m.%Y %H:%M")
        status = "Проверен" if is_checked else "Не проверен"
        text = (
        ...
        await edit_message_if_changed(
            callback=callback,
            text=text,
            reply_markup=await create_report_actions_keyboard(report_id)
        )
        ...
        user = await execute_with_retry_async("SELECT first_name, username, join_date FROM users WHERE user_id = ?", (target_user_id,))
        ban = await execute_with_retry_async("SELECT reason, banned_at, admin_id FROM bans WHERE user_id = ?", (target_user_id,))
        if not user or not ban:
            ...
            return
        first_name, username, join_date = user[0]
        reason, banned_at, admin_id = ban[0]
        join_date_str = datetime.fromtimestamp(join_date).strftime("%d.%m.%Y %H:%M")
    ...
    # Очистка кэша callback после обработки
