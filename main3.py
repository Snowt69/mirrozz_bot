...
from aiogram.utils.markdown import hbold, hcode, hitalic, escape_md
import sys
import os
import shutil
import fcntl
...
async def handle_content(message: types.Message):
    ...
    if state_value == "awaiting_link_content" and await is_admin(user_id):
        ...
        try:
            ...
            user_name = (await execute_with_retry_async("SELECT first_name FROM users WHERE user_id = ?", (new_admin_id,)))[0][0] if await execute_with_retry_async("SELECT first_name FROM users WHERE user_id = ?", (new_admin_id,)) else f"Пользователь {new_admin_id}"
            await send_message_safe(
                bot=bot,
                chat_id=message.chat.id,
                text=f"✅ {user_name} (ID: {new_admin_id}) добавлен в администраторы.",
                reply_markup=await create_back_keyboard()
            )
            await send_message_safe(
                bot=bot,
        ...
        try:
            ...
            await execute_with_retry_async(
                ...
                (new_developer_id, text[1:] if text.startswith("@") else "Unknown", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
            )
            ...
            await send_message_safe(
                bot=bot,
                chat_id=message.chat.id,
                text=f"✅ {user_name} (ID: {new_developer_id}) добавлен в разработчики.",
                reply_markup=await create_back_keyboard()
            )
            await send_message_safe(
                bot=bot,
        ...
        try:
            ...
            required_tables = {'users', 'links', 'reports', 'channels', 'subscriptions', 'stats', 'admins', 'bans', 'developers', 'system_messages'}
            ...
            logger.error(f"Database upload error for user {user_id}: {str(e)}")
            await send_message_safe(
                bot=bot,
                chat_id=message.chat.id,
        ...
        await send_message_safe(
            bot=bot,
            chat_id=message.chat.id,
            text="✅ Системное сообщение отправлено.",
            reply_markup=await create_back_keyboard()
        )
        await dp.storage.set_data(user=user_id, data={})
