...
async def init_database():
    """
    Полная инициализация базы данных со всеми таблицами и индексами
    """
    async with aiosqlite.connect(DATABASE_FILE) as conn:
        # Включаем дополнительные функции SQLite
        ...
        # 10. Таблица системных сообщений
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS system_messages (
                ...
                FOREIGN KEY (sender_id) REFERENCES admins(user_id)
            )
        ''')
        
        # 11. Таблица логов действий
        await conn.execute('''
        ...
        await conn.execute('''
            CREATE INDEX IF NOT EXISTS idx_links_creator 
            ON links(creator_id)
        ''')
        
        await conn.execute('''
            CREATE INDEX IF NOT EXISTS idx_reports_checked 
            ON reports(is_checked)
        ''')
        ...
        await conn.execute('''
            ...
            (id, total_users, link_visits, total_links, last_updated) 
            VALUES (1, 0, 0, 0, ?)
        ''', (int(time.time()),))
        ...
        for admin_id in ADMINS:
            await conn.execute('''
                ...
                (user_id, added_at, added_by, permissions) 
                VALUES (?, ?, ?, ?)
            ''', (admin_id, int(time.time()), admin_id, 'all'))
        
        # Добавляем разработчиков по умолчанию
        for dev_id in DEVELOPER_IDS:
            await conn.execute('''
                INSERT OR IGNORE INTO developers 
                (user_id, username, added_at, access_level) 
                VALUES (?, ?, ?, ?)
            ''', (dev_id, 'admin', datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 3))
...
async def on_startup(dp):
    ...
    while retry_count < max_retries:
        try:
            ...
            retry_count += 1
            logger.error(f"Database error (attempt {retry_count}/{max_retries}): {str(e)}")
